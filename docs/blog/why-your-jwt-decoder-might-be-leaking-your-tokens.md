...
- **Color-coded token parts** — header in blue, payload in green, signature in gold
- **Hover tooltips** explaining each JWT claim
- **Security linter** that flags issues like missing expiry, weak algorithms, or overly long token lifetimes
- **Signature verification** using the Web Crypto API

## What jwx can do

| Feature | Description |
|---------|-------------|
| **Decode** | Beautiful colorized terminal output with rounded boxes |
| **Sign** | Create tokens with HMAC, RSA, ECDSA, EdDSA |
| **Security audit** | Automatic checks for common JWT mistakes |
| **Verify signatures** | Locally, using Web Crypto API in browser |
| **JSON mode** | Machine-readable output for piping to `jq` |
| **Clipboard** | Read tokens directly from your clipboard |
...
These checks run automatically — no configuration needed. In the web decoder, findings appear in a collapsible panel with severity levels (critical, high, medium, low, info).
...
*Tags: JWT, Security, Golang, CLI, Open Source, Privacy*
